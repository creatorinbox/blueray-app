...
<table align="center" width="100%" height='100%'>
    <thead>
      <tr>
          ...
          <th colspan="5" rowspan="1"><b style="text-transform: capitalize;">{{ __('purchase_return_invoice') }}</b></th>
      </tr>
      <tr>
          <th colspan="2" rowspan="1">{{ __('invoice_no') }} : {{ $return_code ?? '' }}</th>  
          <th colspan="3" rowspan="1">{{ __('date') }} : {{ $return_date ? \Carbon\Carbon::parse($return_date)->format('Y/m/d') : '' }}</th>
      </tr>
      <tr>
        <td colspan="5" style="padding-left: 15px;">
          <b>{{ __('supplier_address') }}</b><br/>
          {{ __('name') }}: {{ $supplier_name ?? '' }}<br/>
          @if(!empty($supplier_mobile)){{ __('mobile') }}: {{ $supplier_mobile }}<br>@endif
          @if(!empty($supplier_address)){{ $supplier_address }}@endif
          @if(!empty($supplier_country)),{{ $supplier_country }}@endif
          @if(!empty($supplier_state)),{{ $supplier_state }}@endif
          @if(!empty($supplier_city)),{{ $supplier_city }}@endif
          @if(!empty($supplier_postcode)) -{{ $supplier_postcode }}@endif
          <br>
          @if(!empty($supplier_email)){{ __('email') }}: {{ $supplier_email }}<br>@endif
          @if(!empty($supplier_gst_no)){{ __('gst_number') }}: {{ $supplier_gst_no }}<br>@endif
          @if(!empty($supplier_tax_number)){{ __('tax_number') }}: {{ $supplier_tax_number }}<br>@endif
        </td>
        <td colspan="5" style="padding-left: 15px;">
    ...
    <tbody>
        ...
        @foreach($items as $item)
            @php
                $discount = (!empty($item->unit_discount_per) && $item->unit_discount_per != 0) ? $item->unit_discount_per : 0;
            @endphp
            <tr>
                <td>{{ ++$i }}</td>
    ...
    <tfoot>
      ...
      <tr>
        <td colspan="5" style="height:100px;">
          <b>{{ __('supplier_signature') }}</b><br/>&nbsp;<br/>&nbsp;<br/>&nbsp;<br/>&nbsp;<br/>
        </td>
        <td colspan="5">
