        ...
        $("#"+this_id).attr('disabled',true);  //Enable Save or Update button
         ...
         // alert(result);return;
				result=result.split("<<<###>>>");
					if(result[0]=="success")
					{
						location.href=base_url+"grn/invoice/"+result[1];
					}
...
function return_row_with_data(item_id){
 ...
 // console.log(base_url+"purchase/return_row_with_data/"+rowcount+"/"+item_id+"/"+currency_type+"/"+currency_value);
	$.post(base_url+"grn/return_row_with_data/"+rowcount+"/"+item_id+"/"+currency_type+"/"+currency_value,{},function(result){
        //alert(result);
...
function delete_payment(payment_id){
 if(confirm("Do You Wants to Delete Record ?")){
    var base_url=$("#base_url").val().trim();
    $(".box").append('<div class="overlay"><i class="fa fa-refresh fa-spin"></i></div>');
   $.post(base_url+"grn/delete_payment",{payment_id:payment_id},function(result){
   //alert(result);return;
   result=result.trim();
     if(result=="success")
        {
...
function delete_purchase(q_id)
{
   ...
   if(confirm("Do You Wants to Delete Record ?")){
    $(".box").append('<div class="overlay"><i class="fa fa-refresh fa-spin"></i></div>');
    $.post("grn/delete_purchase",{q_id:q_id},function(result){
   //alert(result);return;
     if(result=="success")
        {
          toastr["success"]("Record Deleted Successfully!");
...
function pay_now(purchase_id){
  $.post('grn/show_pay_now_modal', {purchase_id: purchase_id}, function(result) {
...
function view_payments(purchase_id){
  $.post('grn/view_payments_modal', {purchase_id: purchase_id}, function(result) {
...
function save_payment(purchase_id){
  var base_url=$("#base_url").val().trim();
   ...
   //Validate Input box or selection box should not be blank or empty
    ...
    $.post('grn/save_payment', {purchase_id: purchase_id,payment_type:payment_type,amount:amount,payment_date:payment_date,payment_note:payment_note,paid_status:paid_status}, function(result) {
      result=result.trim();
  //alert(result);return;
        if(result=="success")
        {
...
$.ajax({
  type: "GET",
  url: base_url + "grn/convercurrency", 
  data: {to: $selectedval},
  dataType: "json",  
  cache:false,
  success: 
...
function paymentchange(id){
  ...
  $.post("grn/paymentchange",{id:id},function(result){
    if(result=="success")
    {
      $('#view_payments_modal').modal('toggle');
