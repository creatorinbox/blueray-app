...
$('#save,#update').click(function (e) {
    ...
    if(this_id=="save")  //Save start
    {
					if(confirm("Do You Wants to Save Record ?")){
						...
						$.ajax({
						type: 'POST',
						url: base_url+'expense/newsubcategory',
						data: data,
						cache: false,
						contentType: false,
						processData: false,
             ...
             // alert(result);return;
							if(result=="success")
							{
								window.location=base_url+"expense/sub_category";
								//return;
							}
	...
	else if(this_id=="update")  //Save start
    {
					...
					if(confirm("Do You Wants to Update Record ?")){
						...
						$.ajax({
						type: 'POST',
						url: base_url+'expense/update_sub_category',
						data: data,
						cache: false,
						contentType: false,
						processData: false,
...
function update_status(id,status)
{
	...
	$.post("expense_sub_update_status",{id:id,status:status},function(result){
		result=result.trim();
		if(result=="success")
				{
...
function delete_sub_category(q_id)
{
	
   if(confirm("Do You Wants to Delete Record ?")){
   	$(".box").append('<div class="overlay"><i class="fa fa-refresh fa-spin"></i></div>');
   $.post("delete_sub_category",{q_id:q_id},function(result){
   	result=result.trim();
   //alert(result);return;
	   if(result=="success")
				{
...
function multi_delete(){
    ...
    var this_id=this.id;
		...
		if(confirm("Are you sure ?")){
			...
			$.ajax({
			type: 'POST',
			url: base_url+'expense/multi_sub_delete',
			data: data,
			cache: false,
			contentType: false,
			processData: false,
