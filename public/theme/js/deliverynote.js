  ...
  //Initially flag set true
   ...
   //Validate Input box or selection box should not be blank or empty
    ...
    if( $("#amount").val() == ''){
      ...
      toastr["warning"]("Amount Should not be empty!!");
    ...
    if(parseFloat($("#amount").val()) < 0){
      ...
      toastr["warning"]("Amount Should not be less than 1 !!");
    ...
    if($("#paid_type").val()==''){
      ...
      toastr["warning"]("Please Select Paid Type!!");
    ...
    var this_id=this.id;
        ...
        $("#"+this_id).attr('disabled',true);  //Enable Save or Update button
				...
				url: base_url+'deliverynote/quotationtosales?command='+this_id+'&rowcount='+rowcount+'&tot_subtotal_amt='+tot_subtotal_amt+'&tot_discount_to_all_amt='+tot_discount_to_all_amt+'&tot_round_off_amt='+tot_round_off_amt+'&tot_total_amt='+tot_total_amt+"&other_charges_amt="+other_charges_amt,
   ...
   //Validate Input box or selection box should not be blank or empty
    ...
    var this_id=this.id;
        ...
        $("#"+this_id).attr('disabled',true);  //Enable Save or Update button
				...
				url: base_url+'deliverynote/sales_save_and_update?command='+this_id+'&rowcount='+rowcount+'&tot_subtotal_amt='+tot_subtotal_amt+'&tot_discount_to_all_amt='+tot_discount_to_all_amt+'&tot_round_off_amt='+tot_round_off_amt+'&tot_total_amt='+tot_total_amt+"&other_charges_amt="+other_charges_amt,
         ...
         // alert(result);return;
				...
				result=result.split("<<<###>>>");
...

					{
						...
						location.href=base_url+"deliverynote/invoice/"+result[1];
...
function sales_type_approve(id,statusa){
  ...
  console.log(id);
    ...
    $.post("deliverynote/change_sales_type",{id:id},function(result){
     $(".myModal").html('').html(result);
  ...
  /* $.post("quotation/change_sales_type",{id:id},function(result){
    if(result=="success")
    {
...
function update_approved(id)
{
	...
	$.post("deliverynote/update_approved",{id:id},function(result){
		if(result=="success")
				{
					 toastr["success"]("Status Updated Successfully!");
				  //alert("Status Updated Successfully!");
...
function return_row_with_data(item_id){
  ...
  $("#item_search").addClass('ui-autocomplete-loader-center');
	...
	$.post(base_url+"deliverynote/return_row_with_data/"+rowcount+"/"+item_id,{},function(result){
...
function return_row_with_servicedata(){

  $("#item_search").addClass('ui-autocomplete-loader-center');
document.getElementById("item_search").readOnly = true;
    var base_url=$("#base_url").val().trim();

    var rowcount=$("#hidden_rowcount").val();

    $.post(base_url+"deliverynote/return_row_with_servicedata/"+rowcount,{},function(result){
...

{
   ...
   if(confirm("Do You Wants to Delete Record ?")){
    ...
    $.post("deliverynote/delete_sales",{q_id:q_id},function(result){
...
function save_approval(sales_id){
  ...
  var base_url=$("#base_url").val().trim();
    ...
    var po_number=$("#po_number").val().trim();
  

    $(".box").append('<div class="overlay"><i class="fa fa-refresh fa-spin"></i></div>');

    $(".save_approval").attr('disabled',true);  //Enable Save or Update button

    $.post('deliverynote/save_approval', {sales_id: sales_id,po_date:po_date,po_number:po_number}, function(result) {
       result=result.trim();
         if(result=="success")
        ...
        $(".myModal").attr('disabled',false);  //Enable Save or Update button
...
function save_payment(sales_id){
  ...
  var base_url=$("#base_url").val().trim();
   ...
   //Validate Input box or selection box should not be blank or empty
    ...
    $.post('sales/save_payment', {sales_id: sales_id,payment_type:payment_type,amount:amount,payment_date:payment_date,payment_note:payment_note,cheque_no:cheque_no,cheque_date:cheque_date}, function(result) {
...
if($("#service_stock").val()==1)
{
  ...
  check_field("sales_price");
    ...
    if(this_id=="save_service")  //Save start
    {
          ...
          if(confirm("Do You Wants to Save Record ?")){
            ...
            success: function(result){
              ...
              if(result=="success")
              {
                ...
                window.location=base_url+'deliverynote/add';//"items-view.php";
                return;
              }
              else if(result=="failed")
              {
