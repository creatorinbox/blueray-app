...
$(".send_email").click(function(e){
  ...
  check_field("customer_email");
	...
	console.log(email);
    if (email!='' && !validateEmail(email)) {
            $("#customer_email_msg").html("Invalid Email!").show();
             //flag=false;
             toastr["warning"]("Please Enter valid Email ID.")
			return;
        }
        else{
        	$("#customer_email_msg").html("Invalid Email!").hide();
        }
    var this_id=this.id;
			...
			if(confirm("Are you Sure ?")){
				...
				$.ajax({
				type: 'POST',
				url: base_url+'sales/email_quotation',
				data: data,
				cache: false,
				contentType: false,
				processData: false,
				success: function(result){
					...
					{
					   toastr["error"]("Sorry! Failed to send email.Try again!");
					   failed.currentTime = 0;
						failed.play();
					}
					else
...
/*send email for sales modal end */
