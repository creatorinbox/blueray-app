  ...
  var base_url=$("#base_url").val().trim();
   ...
   //Validate Input box or selection box should not be blank or empty
    ...
    var amount=$("#return_due_amount").val().trim();
            var supplier_notes=$("#supplier_notes").val().trim();
    ...
    //$(".box").append('<div class="overlay"><i class="fa fa-refresh fa-spin"></i></div>');
    //$(".payment_save").attr('disabled',true);  //Enable Save or Update button
    $.post('suppliers/save_return_due_payment', {supplier_id: supplier_id,payment_type:payment_type,supplier_notes:supplier_notes,amount:amount,payment_date:payment_date,payment_note:payment_note}, function(result) {
      result=result.trim();
  //alert(result);return;
        if(result=="success")
        {
