$(document).ready(function(){
	var service_stock = $("#service_stock").val();
	if(service_stock==1)
	{
...
//update status end
//Update Rack No
function add_rack(item_id){
  $.post('items/show_rack_modal', {item_id: item_id}, function(result) {
    $(".item_rack_modal").html('').html(result);
    $('#update_now').modal('toggle');

  });
}
//Update Rack No end
//save rackno
function save_rack_no(item_id){
  var base_url=$("#base_url").val().trim();
...
//alert(rack_no);
    if(rack_no==''){
      toastr["error"]("Please Enter The Rack Number!");
      return false; 
    }

    /*if(rack_no > parseFloat($("#due_amount_temp").html().trim())){
      toastr["error"]("Entered Amount Should not be Greater than Due Amount!");
      return false;
    }*/

   // $(".box").append('<div class="overlay"><i class="fa fa-refresh fa-spin"></i></div>');
   // $(".save_rack_no").attr('disabled',true);  //Enable Save or Update button
    $.post('items/update_rack', {item_id:item_id,item_code:item_code,item_name:item_name,rack_no:rack_no}, function(result) {
      result=result.trim();
  //alert(result);return;
        if(result=="success")
        {
          $('#update_now').modal('toggle');
          toastr["success"]("Rack No Updated Successfully!");
          success.currentTime = 0; 
          success.play();
        ...
        $(".save_rack_no").attr('disabled',false);  //Enable Save or Update button
        $(".overlay").remove();
    });
}
