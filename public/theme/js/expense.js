...
function shift_cursor(kevent,target){
 var expense=parseFloat($("#expense_amt").val().trim());

 	 $("#total_amt").val(expense);
...
getStatesList(category_id);
if(category_id=="8"){
//$("#vechicle_number").attr('disabled',true);  //Enable Save or Update button
document.getElementById("sub_category_name2").style.display = "block";
getStatesList(category_id);
}else{
	document.getElementById("sub_category_name2").style.display = "none";
...
//	alert('hi');
     ...
     var vat=(expense*5)/100;
    totalamt= vat+expense;
    // alert(totalamt);
...
function getStatesList(category_id) {
    $.ajax({
        url: 'getsubcategory',
        type: 'post',
        ...
        dataType: 'json',
        beforeSend: function () {
            jQuery('select#sub_category_name').find("option:eq(0)").html("Please wait..");
        },
        complete: function () {
            // code
        },
        success: function (json) {
            ...
            for (var i = 0; i < json.length; i++) {
                options += '<option value="' + json[i].id + '">' + json[i].sub_category_name + '</option>';
            }
            jQuery("select#sub_category_name").html(options);
